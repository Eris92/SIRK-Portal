(function () {
    ...
    var module = window.SirkPlatformModuleShell.create({
        ...
        render: function (shell) {
            ...
            shell.nav(shell.state.page.primary, [{ key: "incidents", title: "Incidents", icon: "!" }, { key: "email", title: "Email Explorer", icon: "✉" }, { key: "trusted", title: "Tenant Allow/Block", icon: "✓" }, { key: "hunting", title: "Advanced Hunting", icon: "⌕" }], active, function (item) { shell.state.tab = item.key; shell.state.page.tabs.select(item.key, true); });
            shell.state.page.secondary.appendChild(shell.card(active, "Microsoft Defender XDR"));
            if (active === "incidents") return shell.api("incidents").then(function (result) { shell.state.page.details.innerHTML = ""; (result.incidents || []).forEach(function (incident) { shell.state.page.details.appendChild(shell.card(incident.displayName || incident.incidentName || ("Incident " + incident.id), (incident.status || "") + " · " + (incident.severity || ""))); }); });
            if (active === "hunting") {
                var form = window.SharedSettings.form("Advanced Hunting"), label = shell.element("label", "", "KQL query"), input = shell.element("textarea", "", ""), run = shell.element("button", "btn btn-primary", "Run");
                input.rows = 10; run.type = "button"; run.onclick = function () { shell.post("hunt", { query: input.value }).then(function (result) { shell.json(shell.state.page.details, result.result); }).catch(function (error) { shell.error(shell.state.page.details, error); }); };
    ...
    function renderDeviceTable(host, devices) {
        ...
        (devices || []).forEach(function (device) { var row = element("tr"); row.appendChild(element("td", "", text(device.machineName))); row.appendChild(element("td", "", text(device.tenantId) + " / " + text(device.deviceId))); row.appendChild(element("td", "", text(device.agentVersion))); row.appendChild(element("td", device.online ? "is-online" : "is-offline", device.online ? "Online" : "Offline")); row.appendChild(element("td", "", device.lastSeenUtc ? new Date(device.lastSeenUtc).toLocaleString() : "—")); body.appendChild(row); });
        table.appendChild(body); section.appendChild(table); host.appendChild(section);
    }
    function renderCategory(shell, category) {
        var details = shell.state.page.details; clear(details); if (!category) return;
        var heading = element("section", "sirk-card sirk-agent-capability-header"); heading.appendChild(element("h2", "", category.title)); heading.appendChild(element("p", "", category.description || "")); details.appendChild(heading);
        addList(details, "Ustawienia i możliwości", category.settings); addList(details, "Pola danych dostępne w Portalu", category.fields); addList(details, "Źródła danych", category.sources); addList(details, "Ograniczenia i wymagania bezpieczeństwa", category.constraints || category.security);
        if (category.key === "overview") {
            var summary = agentState.snapshot.summary || {}, cards = element("div", "sirk-standalone-grid");
            [["Wszystkie urządzenia", summary.total], ["Online", summary.online], ["Offline", summary.offline], ["Wersje", (summary.versions || []).join(", ") || "—"]].forEach(function (item) { cards.appendChild(shell.card(item[0], text(item[1]))); });

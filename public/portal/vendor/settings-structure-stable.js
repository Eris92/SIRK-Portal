(function () {
    ...
    function waitFor(callback, attempts) {
        ...
        return new Promise(function (resolve) {
            setTimeout(function () { resolve(waitFor(callback, attempts + 1)); }, 30);
        });
    }
    ...
    function loadSnapshot() {
        return fetch("/api/admin/settings", { credentials: "same-origin", cache: "no-store", headers: { Accept: "application/json" } }).then(parse);
    }
    ...
    function field(host, label, value, update) {
        ...
        if (typeof value === "boolean") {
            ...
            input = el("input"); input.type = typeof value === "number" ? "number" : "text";
            input.value = Array.isArray(value) ? value.join(", ") : (value == null ? "" : value);
            input.oninput = function () {
                if (Array.isArray(value)) update(input.value.split(",").map(function (item) { return item.trim(); }).filter(Boolean));
                else update(input.type === "number" ? Number(input.value) : input.value);
            };
        }
    ...
    function editor(host, object, depth) {
        Object.keys(object || {}).sort().forEach(function (key) {
            ...
            if (value && typeof value === "object" && !Array.isArray(value)) {
                var details = el("details", "sirk-card"); details.open = depth === 0;
                details.appendChild(el("summary", "", label));
                var body = el("div", "sirk-settings-stable-form");
                editor(body, value, depth + 1); details.appendChild(body); host.appendChild(details);
            } else {
                field(host, label, value, function (next) { object[key] = next; });
            }
        });
    }
    ...
    function customForm(layout, section, title) {
        ...
        loadSnapshot().then(function (snapshot) {
            ...
            var current = snapshot.moduleSettings.portal[section];
            details.appendChild(el("h2", "", title));
            var form = el("div", "sirk-settings-stable-form"); editor(form, current, 0); details.appendChild(form);
            var actions = el("div", "sirk-settings-stable-actions");
            var save = el("button", "sirk-button", t("Zapisz", "Save"));
            var status = el("span", "sirk-settings-stable-status");
            save.type = "button";
            save.onclick = function () {
                save.disabled = true; status.textContent = t("Zapisywanie…", "Saving…"); status.removeAttribute("data-error");
                saveSnapshot(snapshot).then(function () {
                    status.textContent = t("Zapisano.", "Saved."); save.disabled = false;
                    if (section === "animations" && window.SirkPortalAnimations) window.SirkPortalAnimations.render(current);
                }).catch(function (error) { status.textContent = error.message; status.setAttribute("data-error", "1"); save.disabled = false; });
            };
    ...
    function addPortalEntries(layout, portal) {
        ...
        if (!body || body.querySelector("[data-sirk-stable-extra]")) return;
        [["banner", "Baner", "Banner"], ["maintenance", "Zaślepka serwisowa", "Maintenance page"], ["release", "Release / Co nowego", "Release / What's new"], ["animations", "Animacje", "Animations"]].forEach(function (entry) {
            var button = el("button", "sirk-nav-item sirk-settings-nav-leaf", t(entry[1], entry[2]));
            button.type = "button"; button.setAttribute("data-sirk-stable-extra", entry[0]);
            button.onclick = function () {
                Array.prototype.forEach.call(body.querySelectorAll(".active,.is-active"), function (node) { node.classList.remove("active", "is-active"); });
                button.classList.add("active"); customForm(layout, entry[0], t(entry[1], entry[2]));
            };
    ...
    function showGroup(layout, kind) {
        ...
        if (!modules || !portal) return false;
        ...
        target.classList.remove("sirk-settings-native-hidden"); target.classList.add("sirk-settings-stable-root"); target.open = true;
        if (kind === "portal") addPortalEntries(layout, portal);
        return true;
    }

    function updateActive(primary) {
        var promoted = primary.querySelector(".sirk-settings-stable-primary");
        if (!promoted) return;
        Array.prototype.forEach.call(promoted.querySelectorAll("[data-settings-primary]"), function (button) {
            button.classList.toggle("active", button.getAttribute("data-settings-primary") === activePrimary);
        });
    }
    ...
    function navigate(kind) {
        ...
        var target = kind === "identity" ? native.identity : kind === "server" ? native.server : native.settings;
        ...
        if (kind === "modules" || kind === "portal") {
            ...
            setTimeout(function () { navigationBusy = false; reconcile(); }, 50);
        }
    }
    ...
    function addPrimary(primary) {
        ...
        [["modules", "Moduły", "Modules"], ["portal", "Portal", "Portal"], ["identity", "Użytkownicy i grupy", "Users and groups"], ["server", "Serwer", "Server"]].forEach(function (entry) {
            var button = el("button", "sirk-nav-item", t(entry[1], entry[2]));
            button.type = "button"; button.setAttribute("data-settings-primary", entry[0]);
            button.onclick = function () { navigate(entry[0]); };
            promoted.appendChild(button);
        });
    ...
    function reconcile() {
        ...
        if (activePrimary === "modules" || activePrimary === "portal") showGroup(layout, activePrimary);
    }
    ...
    function mount(host) {
        ...
        var observer = new MutationObserver(function (records) {
            var structural = records.some(function (record) { return record.type === "childList" && (record.addedNodes.length || record.removedNodes.length); });
            if (!structural || navigationBusy) return;
            clearTimeout(remountTimer);
            remountTimer = setTimeout(reconcile, 40);
        });
        observer.observe(host, { childList: true, subtree: true });
        setTimeout(function () { navigate(activePrimary); }, 0);
    }
    ...
    function install() {
        ...
        var wrapped = function (host) { original(host); mount(host); };
        wrapped.__sirkStableWrapped = true;

...
module.exports.createModule = function (context) {
    ...
    function agentDevices() {
        ...
        return Object.keys(registry.devices || {}).map(function (key) {
            ...
            return {
                ...
                quarantine: device.quarantine || null,
                endurance: device.endurance || null,
                activity: device.activity || null,
                browserActivity: device.browserActivity || null,
                risk: device.risk || null,
                tamper: device.tamper || null,
                portalStatus: device.portalStatus || null,
                telemetryQueue: device.telemetryQueue || null,
    ...
    function capabilityCatalog() {
        return [
            ...
            { key: "policy", title: "Polityki", description: "Podpisane ES256 polityki przypisane do tenant/device z anti-replay i anti-rollback.", fields: ["heartbeat"], settings: ["policyId", "epoch", "version", "nonce", "validFrom/validTo", "acknowledgedPolicyIds"], sources: ["active-policy.json", "policy-state.bin", "Archive/Accepted", "Archive/Rejected"] },
            ...
            { key: "endurance", title: "Endurance", description: "Próbki stabilności do 48 godzin, trendy RAM, restarty, przerwy i niezdrowe próbki.", fields: ["endurance"], settings: ["sample interval", "maximum samples", "RAM trend/hour", "leak suspicion"], sources: ["endurance-samples.jsonl", "endurance-summary.json", "endurance-report.html"] },
            { key: "telemetry", title: "Telemetria", description: "Szyfrowana kolejka offline z retry, backoff, retencją i throttlingiem.", fields: ["telemetryQueue", "portalStatus"], settings: ["50 MB", "5000 plików", "14 dni retencji", "około 1 zwykły wpis / 5 min"], sources: ["TelemetryQueue", "agent-telemetry.jsonl", "agent-event-batches.jsonl"] },
            ...
            { key: "quarantine", title: "Kwarantanna i tamper", description: "Fail-closed, trwała kwarantanna DPAPI i wyjście wyłącznie przez podpisaną Recovery Policy.", fields: ["security", "quarantine", "tamper"], settings: ["integrity verification", "quarantine state", "Recovery Policy"], sources: ["security-state.json", "quarantine-state.bin", "quarantine-status.json"] },
            { key: "activity", title: "Kolektory aktywności", description: "Procesy, aktywne okno, idle, metadane schowka, USB, druk, timing myszy/klawiatury, UI Automation i pliki.", fields: ["activity"], settings: ["enabled", "processes", "interactiveContext", "clipboardMetadata", "usb", "printing", "mouseTiming", "keyboardTiming", "uiAutomation", "fileRoots", "intervalSeconds"], constraints: ["domyślnie wyłączone", "bez znaków klawiatury", "bez treści schowka", "pliki tylko z jawnych korzeni polityki"] },
            { key: "browser", title: "Browser Bridge", description: "Edge/Chrome, allowlista domen, aktywna sesja, karty, download, upload i drag-and-drop.", fields: ["browserActivity"], settings: ["allowed domains", "session SID", "tab telemetry", "downloads", "uploads", "drag-and-drop"], constraints: ["wymaga podpisanej polityki", "zakres ograniczony allowlistą"] },
            ...
            { key: "risk", title: "Insider Risk", description: "Korelacja masowych pobrań, archiwizacji, uploadu, USB, druku i usuwania śladów.", fields: ["risk"], settings: ["triggerSource HR/Security", "riskAnalytics", "screenshotOnRisk", "screenshotThreshold"], sources: ["risk-report.json", "risk-report.html", "risk-report-manifest.json"] },
